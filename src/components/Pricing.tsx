    ...
    <section id="pricing" style={{ background: 'var(--black-2)', borderTop: '1px solid var(--border)', padding: 'var(--pad-y) var(--pad-x)' }}>
      <div style={{ maxWidth: 'var(--max-w)', margin: '0 auto' }}>
                ...
                {plan.features.map((f, fi) => (
                  <div key={fi} style={{
                    ...
                    <span style={{
                      ...
                      background: f.active ? (plan.featured ? '#0C0C0C' : 'rgba(230,74,25,0.15)') : 'transparent',
                      border: `1px solid ${f.active ? (plan.featured ? '#0C0C0C' : 'rgba(230,74,25,0.4)') : 'currentColor'}`,
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                    }}>
                      {f.active
                        ? <Check size={10} color={plan.featured ? '#F2F0EB' : '#E64A19'} />
        ...
        <motion.div
          ...
          style={{
            border: '1px dashed rgba(230,74,25,0.28)',
            borderRadius: 24, padding: '4rem 5vw',
            display: 'flex', alignItems: 'center', justifyContent: 'space-between',
            flexWrap: 'wrap', gap: '3rem', background: 'rgba(230,74,25,0.03)',
            transition: 'all 0.4s', cursor: 'pointer'
          }}
          onMouseEnter={(e) => { e.currentTarget.style.background = 'rgba(230,74,25,0.06)'; e.currentTarget.style.transform = 'scale(1.015)'; }}
          onMouseLeave={(e) => { e.currentTarget.style.background = 'rgba(230,74,25,0.03)'; e.currentTarget.style.transform = 'scale(1)'; }}
          onClick={(e) => {
